...
class RoughPoseGuess(Node):
    def __init__(self):
        ...
        # Publisher for rough pose guess
        self.pose_pub = self.create_publisher(PoseStamped, '/rough_pose_guess', 10)
    ...
    def timer_callback(self):
        ...
        if self.map is not None and self.lidar_points is not None:
            ...
            translation = Vector3(trans_matrix[0,3],trans_matrix[1,3],trans_matrix[2,3])
            self.rough_pose_guess = Transform(translation, rotation)
            # Publish message
